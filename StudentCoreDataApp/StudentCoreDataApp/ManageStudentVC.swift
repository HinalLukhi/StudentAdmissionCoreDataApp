...
class ManageStudentVC: UIViewController {
    ...
    @objc func onSegClick(){
        if ClassSelect.selectedSegmentIndex == 0 {
           ...
           studArray = CoreDataHandler.shared.fetchClassWise(sclass: "FYMCA")
            studList.reloadData()
        }else if ClassSelect.selectedSegmentIndex == 2 {
            studArray = CoreDataHandler.shared.fetchClassWise(sclass: "SYMCA")
            studList.reloadData()
        }else{
            studArray = CoreDataHandler.shared.fetchClassWise(sclass: "TYMCA")
            studList.reloadData()
        }
    }
...
extension ManageStudentVC: UITableViewDataSource,UITableViewDelegate{
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        ...
        CoreDataHandler.shared.delete(stud: studArray[indexPath.row]) {
